...
function EditTaskForm({children, className, uid, payload}) {
	...
	if (state.message === 'success') {
		toast(
			<aside className="bg-green-500 text-lime-50 rounded-lg p-6 text-center">
		...
		<section>
			...
			<form action={formAction} className={cn('space-y-5  bg-white    py-8 px-4', className)}>
				...
				<FormControl className="flex flex-col">
					...
					<Input onInput={handleInput} id="category" name="category" placeholder="enter a new category" value={category} />
				</FormControl>

				<FormControl className="flex flex-col">
					<Label htmlFor="task">Task</Label>
					<Input onInput={handleInput} id="task" name="task" placeholder="enter a new task" value={task} />
				</FormControl>
